...
console.log('\nSDK Domain:', JSON.stringify(sdkData.domain));
console.log('SDK Types keys:', Object.keys(sdkData.types));
console.log('SDK Values keys:', Object.keys(sdkData.values));

// Compute hashes
const sdkDomainHash = ethers.TypedDataEncoder.hashDomain(sdkData.domain);
const localDomainHash = ethers.TypedDataEncoder.hashDomain(domain);
console.log('SDK domain hash:', sdkDomainHash);
console.log('Local domain hash:', localDomainHash);
console.log('Domain match:', sdkDomainHash === localDomainHash ? '✅' : '❌');

// Remove extra fields from SDK types if present
const sdkTypesClean = {};
for (const [key, fields] of Object.entries(sdkData.types)) {
  sdkTypesClean[key] = fields.filter(f => f.type !== 'function');
}

const sdkMsgHash = ethers.TypedDataEncoder.from(sdkTypesClean).hash(sdkData.values);
const localTypes = {
  TokenPermissions: [
...
console.log('SDK msg hash:', sdkMsgHash);
console.log('Local msg hash:', localMsgHash);
console.log('Msg hash match:', sdkMsgHash === localMsgHash ? '✅' : '❌');
